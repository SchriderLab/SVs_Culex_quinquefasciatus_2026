...
#build genome file. need the following files from NCBI, and the genome must be added to the snpeff config file:
...
awk -F '\t' 'NR==1 || $2 > “{99th_percentile}”’ sweepfinder_cg_chr3_merged.txt > cg_chr3_99.txt
...
#Note: the IR_genes.bed file created from A. gambiae orthologs info - chrom, start, end (see Supplemental Material)
...
#3. for the high-impact VCFs, we filtered using the following command to isolate variants exerting high-impact effects specifically on IR genes
(
